...
With the use of logs, it’s easier to go back into that specific time frame where the problematic event occurred, discover connections with other events, and diagnose the cause of the issue. 
...
The types of logs you should collect mainly depends on your intentions with log data collection. 
...
- Do you need logs to be stored for a longer period for machine learning and pattern recognition? 
...
But on the other hand, if more memory and CPU is allocated for indexing, performance of other activities like search and analysis can be negatively affected. Therefore, need to find a balance (probably following an empirical approach)  
...
Sometimes it might be beneficial to start without replication and add replicas later, once the time-critical initial indexing has been completed. This is much more efficient (as it’s just a network transfer) than duplicating the indexing process. 
